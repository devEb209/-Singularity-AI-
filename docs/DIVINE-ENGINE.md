...
## Intelligence-first studio

The Divine workspace now combines a real artifact stage, side chat, authenticated attachments, ten presets and a registry of 220 typed settings across rendering, lighting, shadows, reflections, post-processing, camera, input, physics, gameplay, world, terrain, streaming, NPC, animation, audio, VFX, UI, networking, optimization, quality, build and accessibility.

Every setting declares `active`, `adapter-required`, or `architectural`. Low devices render at most 20 controls per page and use a lightweight sandboxed iframe. Active runtime settings are applied with `postMessage` to the WebGL build without reload, providing native edit-while-playing behavior without Java. Other commands are persisted for Master AI planning and are not falsely marked applied.
