...
The user-facing Genesis path is now an executing chain (describe → knowledge → RRW → D-O15 → session → verify → refine). Worlds persist across operations and unify living continuum (kinship, labor, trade, health, speech-as-knowledge) on that same graph. Executing the chain is **not** the same as closing Genesis. `genesisClosed` stays false until the generation actually competes.
