...
The user-facing Genesis path is now an executing chain (describe → knowledge → RRW → D-O15 → session → verify → refine). Worlds persist across operations, unify living continuum, and deepen cycles, reference-Earth processes, biosphere and hydro/cryo/geo/atmo spheres on that same graph. Executing the chain is **not** the same as closing Genesis. `genesisClosed` stays false until the generation actually competes.
