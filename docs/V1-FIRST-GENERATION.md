...
The user-facing Genesis path is now an executing chain (describe → knowledge → RRW → D-O15 → session → verify → refine). Worlds persist across operations, unify living continuum, and deepen known-reality cycles plus reference-Earth processes on that same graph. Executing the chain is **not** the same as closing Genesis. `genesisClosed` stays false until the generation actually competes.
