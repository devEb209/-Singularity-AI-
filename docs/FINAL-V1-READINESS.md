...
Generated: 2026-08-24T23:55:52.882Z
...
- **Advanced VFX production/runtime** — PARTIAL; dependency: CPU Stable Fluids, FNWS heightfield plus shallow-water compute IR and CPU PBR radiance now execute; GPU SPH and vendor shaders remain adapters; activation: Effect artifacts and performance gates
- **Hardware GPU presentation** — ADAPTER_REQUIRED; dependency: WebGPU/Vulkan/DX/Metal device; requestAdapter path exists and does not fake a device; activation: navigator.gpu.requestAdapter → device → present the same IR
