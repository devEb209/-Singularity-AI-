...
## Resource profile compiler

The foundation can now compile a hardware-aware resource profile from RAM, CPU, storage and battery context. Low-end profiles reserve host resources, limit concurrent services, require remote builds and preserve on-demand loading. This is a planning/configuration artifact, not a claim that the OS image is running.
