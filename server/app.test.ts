...
describe('Singularity API', () => {
  ...
  it('creates a real Divine Engine project graph without faking missing 3D artifacts',async()=>{const context=await authenticatedApp();const created=await context.app.inject({method:'POST',url:'/api/v1/divine-engine/projects',headers:context.headers,payload:{name:'Future City',brief:'Open-world futuristic city with vehicles, NPCs and dynamic weather',target:'unreal',executionPolicy:'remote-first',deviceProfile:{tier:'low',memoryGB:1}}});expect(created.statusCode).toBe(201);const body=created.json();expect(body.divine.format).toBe('snb-divine-project-v1');expect(body.divine.status).toBe('blocked');expect(body.bosses).toHaveLength(14);expect(body.bosses[0].boss).toBe('master-ai');expect(body.bosses.at(-1).boss).toBe('build');expect(body.artifacts).toHaveLength(0);expect(body.execution.deviceRole).toBe('terminal');expect(body.execution.preview.viewport).toBe(false);expect(body.pipeline3d.gaps).not.toContain('3d.generate');expect(body.pipeline3d.gaps).not.toContain('3d.retopology');expect(body.pipeline3d.gaps).not.toContain('rig.character');expect(body.pipeline3d.gaps).not.toContain('animation.motion');expect(body.pipeline3d.gaps).toContain('research.web');const list=await context.app.inject({method:'GET',url:'/api/v1/divine-engine/projects',headers:context.headers});expect(list.json().data[0].progress).toBe(0);const prototype=await context.app.inject({method:'POST',url:`/api/v1/divine-engine/projects/${body.divine.id}/prototype-3d`,headers:context.headers,payload:{prompt:'futuristic city blockout'}});expect(prototype.statusCode).toBe(200);expect(prototype.json().result.verification.valid).toBe(true);expect(prototype.json().project.artifacts).toHaveLength(1)})
