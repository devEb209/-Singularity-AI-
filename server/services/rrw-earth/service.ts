...
export class RrwEarthService {
  private core = new RrwEarthCore()
  ...
  async build(userId: string, input: { projectId: string; name: string; prompt?: string }) {
    ...
    return persistUesArtifact(this.store, this.artifacts, this.root, userId, input, payload, {
      suffix: 'rrw-earth.json',
      mime: 'application/vnd.snb.rrw-earth+json',
      type: 'production.rrw-earth',
      producer: 'ues.rrw-earth',
