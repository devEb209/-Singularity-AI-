...
export class UesCorpusService {
  private core = new UesCorpusCore()
  ...
  async compile(userId: string, input: { projectId: string; name: string; prompt?: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'analysis.ues-corpus', producer: 'ues.corpus',
      verification: payload.verification,
      metadata: { count: payload.count, kinds: payload.kinds.length },
      license: 'Apache-2.0',
    })
