...
export const catalog: CorpusEntry[] = [
  ...
  },
  {
    ...
    prompt: 'fuzil FN FAL',
    parts: [
      part('receiver', null, [0, 0.18, 0], [0.18, 0.05, 0.04], 'structure', 'metal'),
      part('barrel', 'receiver', [0.28, 0.19, 0], [0.22, 0.02, 0.02], 'structure', 'metal'),
      part('stock', 'receiver', [-0.22, 0.17, 0], [0.14, 0.04, 0.03], 'support', 'wood'),
      part('mag', 'receiver', [0.02, 0.1, 0], [0.04, 0.08, 0.02], 'structure', 'metal'),
      part('bolt', 'receiver', [0.06, 0.2, 0.02], [0.05, 0.015, 0.015], 'locomotion', 'metal'),
    ],
  },
  {
    id: 'bird-01',
    kind: 'bird',
    prompt: 'ave de rapina',
    parts: [
      part('torso', null, [0, 0.18, 0], [0.12, 0.07, 0.07], 'structure', 'feather'),
      part('head', 'torso', [0.14, 0.22, 0], [0.05, 0.04, 0.04], 'skin', 'beak'),
      part('wing-l', 'torso', [0, 0.2, -0.16], [0.08, 0.02, 0.18], 'locomotion', 'feather'),
      part('wing-r', 'torso', [0, 0.2, 0.16], [0.08, 0.02, 0.18], 'locomotion', 'feather'),
    ],
  },
  {
    id: 'boat-01',
    kind: 'boat',
    prompt: 'barco de pesca',
    parts: [
      part('hull', null, [0, 0.12, 0], [0.7, 0.1, 0.22], 'structure', 'wood'),
      part('mast', 'hull', [0.05, 0.55, 0], [0.03, 0.4, 0.03], 'support', 'wood'),
      part('sail', 'mast', [0.05, 0.62, 0.02], [0.02, 0.28, 0.22], 'structure', 'fabric'),
    ],
  },
]
...
export const byPrompt = (prompt: string) => {
  ...
  if (/fal|fuzil|arma|rifle|weapon/.test(lower)) return catalog.find(item => item.kind === 'weapon')!
  if (/ave|passaro|pássaro|bird/.test(lower)) return catalog.find(item => item.kind === 'bird')!
  if (/barco|boat|navio/.test(lower)) return catalog.find(item => item.kind === 'boat')!
  return catalog[0]
}
