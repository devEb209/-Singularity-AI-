...
export class UesStreamService {
  private core = new UesStreamCore()
  ...
  async compile(userId: string, input: { projectId: string; name: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'runtime.ues-stream', producer: 'ues.stream',
      verification: payload.verification,
      metadata: { budget: payload.budget, gis: false },
      license: 'Apache-2.0',
    })
