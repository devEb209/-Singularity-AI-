...
export class UesImageService {
  private core = new UesImageCore()
  ...
  async build(userId: string, input: { projectId: string; name: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'analysis.ues-image', producer: 'ues.image',
      verification: result.verification,
      metadata: { sr: result.superResolution.method, flow: result.frameFlow.method },
      license: 'Apache-2.0',
    })
