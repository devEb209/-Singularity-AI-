...
export class UesCriticService {
  private core = new UesCriticCore()
  ...
  async compile(userId: string, input: { projectId: string; name: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'analysis.ues-critic', producer: 'ues.critic',
      verification: payload.verification,
      metadata: { intersections: payload.crossing.intersections, vision: false },
      license: 'Apache-2.0',
    })
