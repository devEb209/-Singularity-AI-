...
export class UesRetopoService {
  private core = new UesRetopoCore()
  ...
  async build(userId: string, input: { projectId: string; name: string; prompt?: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'mesh.ues-retopo', producer: 'ues.retopo',
      verification: result.verification,
      metadata: { reduction: result.reduction },
      license: 'Apache-2.0',
    })
