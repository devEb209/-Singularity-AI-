...
export class UesConstraintsService {
  private core = new UesConstraintsCore()
  ...
  async simulate(userId: string, input: { projectId: string; name: string }) {
    ...
    const safe = `${input.name.replace(/[^a-zA-Z0-9._-]/g, '_')}-constraints.json`
    const storagePath = join(directory, `${fileId}-${safe}`)
    await writeFile(storagePath, bytes, { flag: 'wx' })
    const file: FileAsset = {
      id: fileId, userId, projectId: input.projectId, name: safe,
      mimeType: 'application/vnd.snb.ues-constraints+json',
      size: bytes.length,
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'simulation.ues-constraints', producer: 'ues.constraints',
      verification: payload.verification,
      metadata: { distance: payload.distance.after, featherstone: false },
      license: 'Apache-2.0',
    })
