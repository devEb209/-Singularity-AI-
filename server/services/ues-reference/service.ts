...
export class UesReferenceService {
  private core = new UesReferenceCore()
  ...
  async compile(userId: string, input: { projectId: string; name: string; prompt?: string }) {
    ...
    const payload = this.core.process(input.prompt ?? 'personagem humano')
    const bytes = Buffer.from(JSON.stringify(payload, null, 2))
    const directory = join(this.root, userId)
    await mkdir(directory, { recursive: true })
    const fileId = id('file')
    const safe = `${input.name.replace(/[^a-zA-Z0-9._-]/g, '_')}-reference.json`
    const storagePath = join(directory, `${fileId}-${safe}`)
    await writeFile(storagePath, bytes, { flag: 'wx' })
    const file: FileAsset = {
      id: fileId, userId, projectId: input.projectId, name: safe,
      mimeType: 'application/vnd.snb.ues-reference+json',
      size: bytes.length,
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'analysis.ues-reference', producer: 'ues.reference',
      verification: payload.verification,
      metadata: { constraints: payload.constraints.length, vision: false },
      license: 'Apache-2.0',
    })
