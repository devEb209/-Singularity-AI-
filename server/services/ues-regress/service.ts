...
export class UesRegressService {
  private core = new UesRegressCore()
  constructor(private store: Store, private artifacts: ArtifactGraphService, private root = resolve('./data/uploads')) {}

  async compare(userId: string, input: { projectId: string; name: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'analysis.ues-regress', producer: 'ues.regress',
      verification: payload.verification,
      metadata: { rollback: payload.rollback, gpu: false },
      license: 'Apache-2.0',
    })
