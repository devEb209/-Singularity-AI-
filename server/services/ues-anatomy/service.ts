...
export class UesAnatomyService {
  private core = new UesAnatomyCore()
  ...
  async build(userId: string, input: { projectId: string; name: string; prompt?: string }) {
    ...
    const artifact = this.artifacts.register(userId, {
      projectId: input.projectId, fileId: file.id, type: 'rig.ues-anatomy', producer: 'ues.anatomy',
      verification: result.verification,
      metadata: { kind: result.kind, bones: result.verification.boneCount },
      license: 'Apache-2.0',
    })
