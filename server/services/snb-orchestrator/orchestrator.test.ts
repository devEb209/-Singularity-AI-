...
describe('SNB Genesis orchestrator', () => {
  it('runs internal 3D automatically and only issues canonical pending Puter tickets', () => {
    ...
    expect(result.clientPlan.serverExecutesPuter).toBe(false)
    expect(result.clientPlan.automaticWhenPuterPresent).toBe(true)
    expect(result.clientPlan.criticAccepted).toBe(true)
