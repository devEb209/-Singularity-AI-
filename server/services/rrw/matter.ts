...
export const scatterSpectrum = (substanceId: string, path: number, incident: Record<SpectrumBand, number>, extraScatter = 0) => {
  const substance = requireSubstance(substanceId)
  return Object.fromEntries(spectrumBands.map(band => {
    const extinction = substance.optical.absorption[band] + substance.optical.scattering[band] + extraScatter
    return [band, beerLambert(incident[band], extinction, path)]
  })) as Record<SpectrumBand, number>
}
