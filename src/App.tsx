...
function Home({ onNavigate, notify }: { onNavigate: (id: string) => void; notify: (s: string) => void }) {
  ...
  return <div className="home-page page-enter">
    <section className="hero">
      ...
      <Composer onSubmit={async text => { try { const project = await api.createProject(text.trim().slice(0, 72), text.trim()); const result = await api.compileMasterIntelligence(project.id, text.trim()); notify(`Master Intelligence · ${result.program.specialists.length} especialistas · cenário ${result.program.decision.selectedScenario}`); onNavigate('mission-control') } catch (error) { notify(error instanceof Error ? error.message : 'Não foi possível compilar o objetivo') } }} />
